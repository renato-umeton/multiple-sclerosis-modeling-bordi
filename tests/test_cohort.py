...
from msrelapse.model import DoubleWell, barrier_ratio_from_durations, fold_beta
...
# The band fraction and the time step the stochastic cohorts of this file run
# at, rather than the defaults of CohortSpec. The default step of 0.02 weeks and
# band fraction of 0.3 leave the two biases that msrelapse.simulate documents
# for itself: an episode measured on the integration grid runs about a tenth
# long, and about one simulated remission in seven is shorter than a week and
# merges the relapses around it into one longer weekly episode. Together they
# put the weekly relapse mean about a third above its target, which no
# calibration correction inside this module can remove. A shorter step and a
# wider hysteresis band shrink both: at these values fewer than one remission in
# twenty falls inside a week.
SDE_BAND_FRACTION = 0.5
SDE_DT = 0.01

# What is left once the rounding correction is applied: the merging inflation
# above, measured between 3 and 15 percent on the relapse side over a dozen
# seeds, and the sampling noise of about a thousand relapses, a few percent
# more. The uncorrected engine misses by close to 30 percent, so the tolerance
# still separates a corrected cohort from an uncorrected one, which is what
# test_the_rounding_correction_shortens_the_weekly_relapse checks directly.
SDE_TOLERANCE = 0.2
...
def sde_spec(engine: Engine = "sde", weekly: bool = True) -> CohortSpec:
    """Return the cohort of the specification, at the finer grid of this file."""
    return CohortSpec(
        n=120,
        ...
        engine=engine,
        weekly=weekly,
        band_fraction=SDE_BAND_FRACTION,
        dt=SDE_DT,
    )
...
def test_the_paper_follow_up_windows_bias_the_remission_mean_low(paper_cohort: Cohort) -> None:
    ...
    # msrelapse.fit.fit_durations with censoring is the correction.
    assert paper_cohort.durations is not None
    mean = naive_mean(paper_cohort.durations, HEALTH)
    assert 0.6 * TAU_HEALTH < mean < 0.95 * TAU_HEALTH
...
def test_paper_cohort_follow_up_overlaps_the_printed_range(paper_cohort: Cohort) -> None:
    follow_up = paper_cohort.patients["followup_weeks"]
    assert follow_up.min() <= PAPER.rr_phase_max_weeks.value
    assert follow_up.max() >= PAPER.rr_phase_min_weeks.value
