...
- Data: the de-identified weekly series cannot be released without a decision from
  the co-authors, so the package ships a synthetic twin generated by the renewal
  engine from mean durations solved so that the naive means reproduce the printed
  pair, with loud provenance, plus instructions for requesting the original. The
  twin uses no potential and no fitted beta or sigma; those belong to the sde
...
- The ORCID and the archive DOI are left out of CITATION.cff and codemeta.json
  rather than filled with a placeholder, because an invalid ORCID fails cff
  validation and an empty identifier misleads a harvester; each file carries a
  comment saying where the value goes. The one literal placeholder that ships is
  the Zenodo DOI in the software BibTeX entry of src/msrelapse/_citation.py, with
  a matching commented out badge in README.md, both named by the release steps of
...
compliance and then for code quality before the wave is committed.
