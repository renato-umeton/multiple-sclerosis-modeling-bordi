...
- **pre-commit** runs ruff and mypy. Never bypass the hooks.
...
The four notebooks are generated by `notebooks/build_notebooks.py`, which holds
the source of every cell, and they are committed with the outputs of a run in
them, so that GitHub renders their tables and figures. Rebuild them with

```bash
uv run --group notebooks python notebooks/build_notebooks.py
```

That runs each notebook and writes it with the outputs of that run, and it needs
the `notebooks` group, which supplies nbclient and the kernel. Nothing is
written until all four have run, so a rebuild that stops on a raising cell
leaves the files as they were. A pull request that edits a notebook has to
rebuild it, so that the committed outputs are the ones the committed sources
produce.

Rebuild on the interpreter continuous integration runs the notebooks under,
which is Python 3.13. Every committed figure carries the version of matplotlib
that drew it, stamped inside the image, and the lock file resolves an older
matplotlib below Python 3.11, so a rebuild there rewrites every image in the
diff for no visible change.

Rebuilding is largely yours to remember. What the suite checks is that the
cells, the identifiers and the metadata of every committed file are the ones
the builder writes, that the run counts are those of one run from top to
bottom, that no cell carries a traceback or a warning block, and that each
notebook still runs end to end and renders, cell by cell, the same tables and
figures the committed file shows. It also holds each file under 3 MB and each
notebook to at least the number of rendered tables and figures it carries
today, so a figure drawn at a printing resolution, or one that disappeared,
fails there. What nothing compares is the values: a number that moved, or a
figure drawn a new way, keeps the shape of a notebook, so a change under one, a
new style in `msrelapse.plots` for instance, leaves the committed images stale
with every test still green.

Pass `--no-execute` to write the notebooks without running them, which is for
editing a cell and reading the diff rather than for committing.

Execute them headlessly the way continuous integration does:
